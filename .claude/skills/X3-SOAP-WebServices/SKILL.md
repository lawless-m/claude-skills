...
- **A `Subprog` exits at `End`. Never use `Return` in a Subprog.** `Return` is the exit from a `Gosub`; a `Return` with no matching `Gosub` pops the wrong frame in the SOAP pool channel process. The service then runs exactly ONCE per channel, and every later call on that channel comes back HTTP 200 / status 0 / `poolExecDuration` ~1 ms with no outputs and nothing done -- until the pool is restarted. Nothing in the trace says why. (Proven by A/B 2026-09-14: identical subprograms with and without `Return`; Datel's 17 shipped subprograms contain zero `Return`s.) For early exits use a flag (`WOK`) and nested `If`, and fall through to the single `End`.
...
Single fall-through exit at `End`, early exits via a flag. **No `Return`** --
see the gotchas: a `Return` in a Subprog corrupts the pool channel and the
service silently stops working after one call per channel.
